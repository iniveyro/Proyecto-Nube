...
FOR %%x IN ("D:\Ivan\DolorDeHuebo\SV\UniServer\home\us_pear\PEAR") DO (if %%x=="%PMTPMTPMT%" GOTO :NOTINSTALLED)

REM Check PEAR global ENV, set them if they do not exist
IF "%PHP_PEAR_INSTALL_DIR%"=="" SET "PHP_PEAR_INSTALL_DIR=D:\Ivan\DolorDeHuebo\SV\UniServer\home\us_pear\PEAR"
IF "%PHP_PEAR_BIN_DIR%"=="" SET "PHP_PEAR_BIN_DIR=D:\Ivan\DolorDeHuebo\SV\UniServer\home\us_pear\bin"
IF "%PHP_PEAR_PHP_BIN%"=="" SET "PHP_PEAR_PHP_BIN=D:\Ivan\DolorDeHuebo\SV\UniServer\usr\local\php\php.exe"
GOTO :INSTALLED
